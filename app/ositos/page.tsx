...
import {
  obtenerTarifasBears,
  registrarMovimientoBears,
  registrarBaseCajaBears,
  crearPedidoInsumosBears,
  TarifasBears,
} from '@/lib/bearsQueries';
import { obtenerUsuariosOperarios, registrarNominaYCambioTurno, obtenerSaboresViva } from '@/lib/vivaQueries';

// LISTAS EXTRAÍDAS Y ADAPTADAS PARA 12 FRIENDLY BEARS
...
export default function BearsPage() {
  ...
  const [tarifas, setTarifas] = useState<TarifasBears>({
    subsidio: 9600,
    transporte: 8400,
  ...
  // INVENTARIO BEARS: TOTAL DE PALETAS Y EMPAQUES/VASOS
  ...
  const SEDE_ID_BEARS = 4;

  // EVALUACIÓN DE TURNO COMPLETO / CIERRE
  const esTurnoCierre = (() => {
  ...
  async function handleGuardarBase() {
    ...
    setGuardando(true);
    try {
      const exito = await registrarBaseCajaBears(sesion?.sede_id || SEDE_ID_BEARS, sesion?.usuario_id, monto);
      if (exito) {
        setBaseGuardada(true);
  ...
  async function handleGuardarInventario() {
    ...
    setGuardando(true);
    try {
      const exito = await registrarMovimientoBears(
        sesion.sede_id || SEDE_ID_BEARS,
        sesion.usuario_id,
        tipoMovimiento,
        cantTotal,
  ...
  async function handleGuardarPedidoInsumos() {
    ...
    try {
      const ok = await crearPedidoInsumosBears(
        sesion?.sede_id || SEDE_ID_BEARS,
        sesion?.usuario_id,
        categoriaPedido,
        detalleItems,
  ...
  async function handleGuardarNominaTurno() {
    ...
    const ok = await registrarNominaYCambioTurno({
      sedeId: sesion?.sede_id || SEDE_ID_BEARS,
  ...
  async function handleConfirmarEntrante() {
    ...
    if (
      ...
      const nuevaSesion = {
        ...
        sede_id: sesion?.sede_id || SEDE_ID_BEARS,
        turno: turnoRecibido,
      };
  ...
  if (cargando) {
    return (
      <main className="min-h-screen bg-[#07090e] flex items-center justify-center text-gray-400 text-xs font-bold font-sans">
        Cargando Sede 12 Friendly Bears...
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#07090e] text-slate-100 p-4 font-sans max-w-6xl mx-auto space-y-4 relative">
      ...
      <header className="bg-[#0d111a] border border-gray-800 p-4 rounded-2xl flex justify-between items-center shadow-lg">
        <div>
          <h1 className="text-base md:text-lg font-black text-white tracking-wide">🐻 12 FRIENDLY BEARS</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
        ...
        <div className="bg-[#0d111a] border border-gray-800 p-4 rounded-2xl space-y-4 shadow-md">
          ...
          {/* LISTADO DE ITEMS A CONTAR EN BEARS */}
