...
import {
  ...
  registrarBaseCajaOsitos,
  crearPedidoInsumosOsitos,
  obtenerUsuariosOperarios,
  registrarNominaYCambioTurnoOsitos,
  obtenerSaboresOsitos,
  TarifasOsitos,
} from '@/lib/ositosQueries';
...
export default function OsitosPage() {
  const router = useRouter();
  const [sesion, setSesion] = useState<any>(null);

  const [tarifas, setTarifas] = useState<TarifasOsitos>({
    subsidio: 9600,
    transporte: 8400,
  ...
  const paletasFiltradas = saboresOsitos.filter((p) => {
  ...
  async function crearNuevoProductoBD() {
    ...
    const payload = {
      ...
      sede_id: esProductoGlobal ? 0 : SEDE_ID_OSITOS,
      activo: true,
    };
  ...
  function handleKeyDownTotal(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === 'Enter') {
      e.preventDefault();
      inputsRef.current[`empaque_${LISTA_EMPAQUES_OSITOS[0]}`]?.focus();
    }
  }
  ...
  async function handleGuardarBase() {
    ...
    const sedeId = sesionActual?.sede_id || SEDE_ID_OSITOS;
    ...
    setGuardando(true);
    try {
      const exito = await registrarBaseCajaOsitos(sedeId, usuarioId, monto, turnoId);
      if (exito) {
        setBaseGuardada(true);
  ...
  async function handleGuardarInventario() {
    ...
    const sedeId = sesion?.sede_id || SEDE_ID_OSITOS;

    setGuardando(true);
    try {
      const exito = await registrarMovimientoOsitos(
        sedeId,
        usuarioId,
        tipoMovimiento,
        totalPaletasSuma,
  ...
  async function handleGuardarPedidoInsumos() {
    ...
    Object.entries(cantidadesPedidoPaletas).forEach(([saborId, cant]) => {
      ...
      if (num > 0) {
        const saborObj = saboresOsitos.find((s) => s.id === Number(saborId));
    ...
    try {
      const ok = await crearPedidoInsumosOsitos({
        sedeId,
        usuarioId,
        paletas: paletasObj,
        richi: richiObj,
  ...
  async function handleGuardarNominaTurno() {
    ...
    const ok = await registrarNominaYCambioTurnoOsitos(datosPayload);
    setGuardando(false);
  ...
  async function handleConfirmarEntrante() {
    ...
    if (
      ...
      const nuevaSesion = {
        ...
        sede_id: sesion?.sede_id || SEDE_ID_OSITOS,
        ...
        <div className="bg-[#0b2b48] border border-[#0066b3] p-4 rounded-2xl space-y-4 shadow-md">
          ...
          <div className="bg-[#0e385e] p-3 rounded-xl border border-[#0066b3]/60 space-y-2">
            ...
            <div className="space-y-1.5 max-h-[220px] overflow-y-auto pr-1">
              {LISTA_EMPAQUES_OSITOS.map((item, idx) => (
                <div key={item} className="flex justify-between items-center bg-[#051829] p-2 rounded-lg border border-[#0066b3]">
                  ...
                  <input 
                    ...
                    onKeyDown={(e) => handleKeyDownPedido(e, idx, LISTA_EMPAQUES_OSITOS, 'empaque')}
                    onFocus={(e) => e.target.select()} 
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                >
                  ...
                  <option value="local">🏢 Exclusivo de Sede Ositos</option>
                </select>
              </div>
            </div>
          ...
          <div className="bg-[#0b2b48] border border-[#0066b3] p-6 rounded-3xl max-w-sm w-full space-y-5 shadow-2xl text-center">
            ...
            <div className="space-y-1 border-b border-[#0066b3]/50 pb-3">
              ...
              <h3 className="text-lg font-black text-white tracking-wide">
                Recepción de Turno — 12 Friendly Bears
              </h3>
              <p className="text-xs text-sky-200 font-medium">
