...
export default function OsitosPOSPage() {
  ...
  const [cantidadesInventario, setCantidadesInventario, limpiarCantidadesInv] = useAutoSave<{ [item: string]: number | '' }>('ositos_cantidadesInv', {});
  const [observacionesInventario, setObservacionesInventario, limpiarObsInv] = useAutoSave<string>('ositos_obsInv', '');
  ...
  const [tabPedido, setTabPedido] = useState<CategoriaTab>('richi');
  ...
  const [observacionPedido, setObservacionPedido, limpiarObsPedido] = useAutoSave<string>('ositos_obsPedido', '');

  const [listaGastos, setListaGastos] = useState<{ id: string; concepto: string; monto: number; hora: string }[]>([]);
  const [conceptoGasto, setConceptoGasto, limpiarConceptoGasto] = useAutoSave<string>('ositos_conceptoGasto', '');
  const [montoGasto, setMontoGasto, limpiarMontoGasto] = useAutoSave<number | ''>('ositos_montoGasto', '');
  ...
  const [efectivoDejadoTurno, setEfectivoDejadoTurno] = useState<number | ''>('');
  ...
  const [nuevoProdCategoria, setNuevoProdCategoria] = useState('Richi / Empaque');
  ...
  const listaEmpaquesFiltrados = empaquesBD
    .map((e) => e.nombre)
    .filter((n) => n && n.toLowerCase().trim() !== 'total paletas')
    .sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }));

  const listaEmpaquesCompleta = empaquesBD
    .map((e) => e.nombre)
    .filter((n) => n && n.toLowerCase().trim() !== 'total paletas')
    .sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }));
  ...
  useEffect(() => {
    const sesionLocal =
      localStorage.getItem('ositos_session') ||
      localStorage.getItem('session') ||
      localStorage.getItem('martineto_session');
  ...
  useEffect(() => {
    const channelMesas = supabase
      .channel('schema-db-changes-mesas-ositos')
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'mesa',
          filter: `sede_id=eq.${SEDE_ID_OSITOS}`,
        },
        (payload) => {
    ...
    const channelEmpaques = supabase
      .channel('schema-db-changes-empaques-ositos')
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'empaques_ositos',
        },
        (payload) => {
  ...
  async function descontarStockEmpaquesPorVentaBD(itemsVendidos: any[]) {
    try {
      for (const item of itemsVendidos) {
        ...
        empaquesBD.forEach((emp) => {
          const empNomLimpio = String(emp.nombre || '').toLowerCase().trim();
          if (empNomLimpio && empNomLimpio !== 'total paletas' && norm.includes(empNomLimpio)) {
            empaquesAfectados.push({ nombre: emp.nombre, cantidad: cantidadVendida });
          }
        });

        if (norm.includes('expresso')) {
          empaquesAfectados.push({ nombre: 'Vaso 4 onzas', cantidad: cantidadVendida });
        } else if (
          norm.includes('tinto') ||
          norm.includes('americano') ||
          norm.includes('capuchino') ||
          norm.includes('latte') ||
          norm.includes('milo') ||
          norm.includes('kit kat')
        ) {
          empaquesAfectados.push({ nombre: 'Vaso 7 onzas', cantidad: cantidadVendida });
        } else if (
          norm.includes('granizado') ||
        ...
        } else if (norm.includes('wafleoso') || norm.includes('waffle') || norm.includes('corralito')) {
          empaquesAfectados.push({ nombre: 'Caja Wafleoso y Helado', cantidad: cantidadVendida });
        } else if (norm.includes('yogoso')) {
          empaquesAfectados.push({ nombre: 'Estuche Oso', cantidad: cantidadVendida });
        }
      ...
      console.error('Error descontando stock en empaques_ositos:', e);
    }
  }
  ...
  async function cargarInicial(sesionActual: any) {
    ...
    try {
      ...
      if (listaOpBD && listaOpBD.length > 0) {
        setListaOperarios(listaOpBD);
        setOperarioEntranteId(String(listaOpBD[0].id));
      }
      ...
      if (sedesBD && sedesBD.length > 0) {
        ...
        setSedesSeleccionadasProd([SEDE_ID_OSITOS]);
      } else {
        setListaSedesBD([
          { id: SEDE_ID_OSITOS, nombre: 'Ositos' },
          { id: 1, nombre: 'Viva' },
          { id: 2, nombre: 'Centro' },
      ...
      const { data: nominasBD } = await supabase
        .from('nomina')
        .select('id, monto, fecha, usuario_id, usuario(nombre_completo)')
        .eq('sede_id', SEDE_ID_OSITOS)
        .gte('fecha', inicioDia)
        .order('id', { ascending: true });
      ...
      if (nominasBD && nominasBD.length > 0) {
        const nominasMapeadas: RegistroNominaDia[] = nominasBD.map((n: any) => {
          const nomUser = n.usuario?.nombre_completo || 'Operario';
      ...
      if (listaMesas.length === 0) {
        listaMesas = [
          { id: 301, nombre: 'Mesa 1', estado: 'libre', sede_id: SEDE_ID_OSITOS },
          { id: 302, nombre: 'Mesa 2', estado: 'libre', sede_id: SEDE_ID_OSITOS },
          { id: 303, nombre: 'Mesa 3', estado: 'libre', sede_id: SEDE_ID_OSITOS },
          { id: 304, nombre: 'Mesa 4', estado: 'libre', sede_id: SEDE_ID_OSITOS },
          { id: 305, nombre: 'Mesa 5', estado: 'libre', sede_id: SEDE_ID_OSITOS },
          { id: 306, nombre: 'Corredor 1', estado: 'libre', sede_id: SEDE_ID_OSITOS },
          { id: 307, nombre: 'Corredor 2', estado: 'libre', sede_id: SEDE_ID_OSITOS },
        ];
      }
      ...
      if (prodsVentaBD && prodsVentaBD.length > 0) {
        const prodsVentaLimpios = prodsVentaBD
          .filter((p: any) => {
            const nom = String(p.nombre || p.Nombre || '').toLowerCase();
            const cat = String(p.categoria || p.Categoria || p.CATEGORIA || '').toLowerCase();
            return !nom.includes('paleta') && !cat.includes('paleta');
          })
          .map((p: any) => {
      ...
      const { data: prodsInsumosBD } = await supabase
        .from('producto')
        .select('*')
        .or(`sede_id.eq.${SEDE_ID_OSITOS},sede_id.eq.0,sede_id.is.null`);

      if (prodsInsumosBD) {
        const productosMapeados = prodsInsumosBD
          .filter((p: any) => {
            const cat = String(p.categoria || p.Categoria || '').toLowerCase();
            return !cat.includes('paleta');
          })
          .map((p: any) => ({
            ...p,
  ...
  async function pagarNominaSolo() {
    ...
    try {
      ...
      const nuevoRegistroNomina: RegistroNominaDia = {
        ...
        nombreOperario: sesion?.nombre || sesion?.nombre_completo || 'Operario',
        monto: totalPago,
  ...
  async function handleConfirmarEntranteYCambiarTurno() {
    ...
    if (efectivoDejadoTurno === '' || efectivoDejadoTurno === null || efectivoDejadoTurno === undefined) {
      alert('⚠️ Ingresa el efectivo contado que dejas en caja.');
      return;
    }
    ...
    try {
      const { data: operarioBD, error: errOp } = await supabase
        .from('usuario')
        .select('*')
      ...
      if (errOp || !operarioBD) {
        alert('❌ Operario no encontrado en la tabla "usuario".');
        return;
      }

      const pinCorrecto = String(operarioBD.codigo_acceso || operarioBD.clave || operarioBD.pin || operarioBD.password || '');
      ...
      if (cajaIdActual) {
        await supabase
          .from('caja')
          .update({ efectivo_fisico: Number(efectivoDejadoTurno) })
          .eq('id', cajaIdActual);
      }
      ...
      const nuevaSesion = {
        ...
        id: operarioBD.id,
        nombre: operarioBD.nombre_completo || operarioBD.nombre,
        turno_id: nuevoTurnoId,
        turno_nombre: nuevoTurnoNombre,
      };

      localStorage.setItem('ositos_session', JSON.stringify(nuevaSesion));
  ...
  async function handleGuardarInventario() {
    ...
    if (tipoMovimiento !== 'apertura') {
      const sumaCantidades = Object.values(detalleEmpaquesLimpio).reduce((acc, curr) => acc + curr, 0);
      if (sumaCantidades === 0) {
        alert('⚠️ Ingresa al menos una cantidad mayor al 0 para registrar este movimiento.');
        return;
      }
    }
  ...
  async function enviarPedidoBodega() {
    ...
    if (totalItems === 0) {
      alert('⚠️ Por favor ingresa al menos un producto con cantidad mayor al 0.');
      return;
    }
  ...
  const calcularDisponibilidadActual = (nombreEmpaque: string) => {
    ...
    } else if (nombreEmpaque === 'Vaso 7 onzas') {
      ocupadasEnMesas = todasLasMesasYPedidos.reduce((acc, i) => {
        const n = (i.nombre || '').toLowerCase();
        if (
          n.includes('tinto') ||
          n.includes('americano') ||
          n.includes('capuchino') ||
          n.includes('latte') ||
          n.includes('milo') ||
          n.includes('kit kat')
        ) {
    ...
    } else if (nombreEmpaque === 'Vaso 4 onzas') {
      ocupadasEnMesas = todasLasMesasYPedidos.reduce((acc, i) => {
        const n = (i.nombre || '').toLowerCase();
        if (n.includes('expresso')) {
          return acc + Number(i.cantidad || 1);
        }
        return acc;
      }, 0);
    } else if (nombreEmpaque === 'Caja Wafleoso y Helado') {
      ocupadasEnMesas = todasLasMesasYPedidos.reduce((acc, i) => {
        const n = (i.nombre || '').toLowerCase();
        if (n.includes('wafleoso') || n.includes('waffle') || n.includes('corralito')) {
          return acc + Number(i.cantidad || 1);
        }
        return acc;
      }, 0);
    } else if (nombreEmpaque === 'Estuche Oso') {
      ocupadasEnMesas = todasLasMesasYPedidos.reduce((acc, i) => {
        const n = (i.nombre || '').toLowerCase();
        if (n.includes('yogoso')) {
          return acc + Number(i.cantidad || 1);
        }
        return acc;
      }, 0);
  ...
  async function agregarProductoAMesa(producto: any, esParaLlevar: boolean = false) {
    ...
    if (nombreNorm.includes('expresso')) {
      const dispV4 = calcularDisponibilidadActual('Vaso 4 onzas');
      if (dispV4 <= 0) {
        alert('⚠️ No hay stock disponible de Vaso de 4 onzas (Disponibles: 0).');
        return;
      }
    } else if (
      nombreNorm.includes('tinto') ||
      nombreNorm.includes('americano') ||
      nombreNorm.includes('capuchino') ||
      nombreNorm.includes('latte') ||
      nombreNorm.includes('milo') ||
      nombreNorm.includes('kit kat')
    ) {
      const dispV7 = calcularDisponibilidadActual('Vaso 7 onzas');
      if (dispV7 <= 0) {
        alert('⚠️ No hay stock disponible de Vaso de 7 onzas (Disponibles: 0).');
        return;
      }
    } else if (
      nombreNorm.includes('granizado') ||
      ...
      const dispV12 = calcularDisponibilidadActual('Vaso 12 onzas');
      if (dispV12 <= 0) {
        alert('⚠️ No hay Vasos de 12 onzas disponibles en inventario (Disponibles: 0).');
        return;
      }
    } else if (nombreNorm.includes('wafleoso') || nombreNorm.includes('waffle') || nombreNorm.includes('corralito')) {
      const dispCaja = calcularDisponibilidadActual('Caja Wafleoso y Helado');
      if (dispCaja <= 0) {
        alert('⚠️ No hay stock disponible de Caja Wafleoso y Helado (Disponibles: 0).');
        return;
      }
    } else if (nombreNorm.includes('yogoso')) {
      const dispEstuche = calcularDisponibilidadActual('Estuche Oso');
      if (dispEstuche <= 0) {
        alert('⚠️ No hay stock disponible de Estuche Oso (Disponibles: 0).');
        return;
      }
    }
  ...
  const listaAuditoriaInventario = listaEmpaquesCompleta.map((nombreProd) => {
    ...
    const entradasAdicionales = movimientosDiaBD
      ...
      .reduce((acc, m) => acc + Number(m.detalle?.[nombreProd] || 0), 0);
    ...
    } else if (nombreProd === 'Vaso 7 onzas') {
      cantVendidas = ventasDiaBD.reduce((accV, v) => {
        const items = v.items || [];
        return (
          accV +
    ...
    } else if (nombreProd === 'Vaso 4 onzas') {
      cantVendidas = ventasDiaBD.reduce((accV, v) => {
        const items = v.items || [];
        return (
          accV +
    ...
    } else if (nombreProd === 'Caja Wafleoso y Helado') {
      cantVendidas = ventasDiaBD.reduce((accV, v) => {
        const items = v.items || [];
        return (
          accV +
    ...
    } else if (nombreProd === 'Estuche Oso') {
      cantVendidas = ventasDiaBD.reduce((accV, v) => {
        const items = v.items || [];
        return (
          accV +
  ...
  async function guardarCierreDefinitivoBD() {
    ...
    if (efectivoContadoCierre === '' || efectivoContadoCierre === null || efectivoContadoCierre === undefined) {
    ...
    try {
      ...
      const { error: errorCierreInv } = await supabase.from('inventario_diario').insert([
        {
          sede_id: SEDE_ID_OSITOS,
          usuario_id: usuarioId ? Number(usuarioId) : null,
          ...
          total_paletas: 0,
          detalle_paletas: {},
          detalle_empaques: detalleEmpaquesCierre,
          observaciones: 'Cierre de turno y cuadre de inventario físico',
          turno_id: turnoId,
          fecha_registro: obtenerFechaHoraColombia()
        }
      ]);

      if (errorCierreInv) {
        throw new Error('Error guardando el movimiento de cierre en inventario_diario: ' + errorCierreInv.message);
      }
      ...
      localStorage.removeItem('ositos_session');
    ...
    <main className="min-h-screen bg-[#004e8c] text-[#f1f5f9] p-4 font-sans max-w-[1600px] mx-auto space-y-4 relative">
      ...
      <nav className="sticky top-2 z-40 flex items-center gap-2 bg-[#0b2b48]/95 backdrop-blur-md p-2 rounded-2xl border border-[#0066b3] shadow-xl overflow-x-auto no-scrollbar">
        ...
        <button
          ...
          className={`px-4 py-2.5 rounded-xl text-xs font-black uppercase whitespace-nowrap transition-all cursor-pointer ${
            moduloActivo === 'cierre' ? (esTurnoManana ? 'bg-amber-600 text-white shadow-md' : 'bg-purple-600 text-white shadow-md') : 'bg-[#051829] text-sky-300 border border-[#0066b3] hover:text-white'
          }`}
        >
          {esTurnoManana ? '5. 🔄 Cambio de Turno' : '5. 🌙 Cierre'}
        </button>
      </nav>
        ...
        <div className="bg-[#0b2b48] border border-[#0066b3] p-5 rounded-2xl space-y-4 shadow-md max-w-2xl mx-auto">
          <div className="bg-[#051829] border-2 border-emerald-400/70 p-4 rounded-xl space-y-3 shadow-inner">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] text-sky-300 block mb-1 font-bold">Base de Caja ($):</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[10px] text-amber-300 block mb-1 font-bold">Efectivo inicio de turno (Referencia):</label>
                <div className="w-full bg-[#0e385e] border border-amber-500/50 text-amber-300 font-black text-sm rounded-xl p-2.5 flex items-center">
                  {formatearMoneda(baseCaja)}
                </div>
              </div>
            </div>
            <div className="pt-1">
              <button
                ...
                disabled={baseGuardada || guardandoBase}
                className={`w-full font-bold py-2.5 rounded-xl text-xs transition-all ${baseGuardada ? 'bg-emerald-950 text-emerald-300 border border-emerald-500 cursor-default' : guardandoBase ? 'bg-emerald-800 text-white cursor-not-allowed opacity-75' : 'bg-emerald-600 hover:bg-emerald-500 text-white cursor-pointer'}`}
              >
          ...
          <div className="flex justify-between items-center border-b border-[#0066b3]/50 pb-2 pt-2">
            <h2 className="text-sm font-black text-white flex items-center gap-1.5">
              📦 Conteo Físico de Empaques e Inventario
            </h2>
          ...
          <div className="space-y-1">
            ...
            >
              ...
              <option value="nuevas">📦 Empaques / Insumos Nuevos (Ingreso)</option>
          ...
          <div className="space-y-1.5 max-h-[350px] overflow-y-auto pr-1">
            <span className="text-xs text-sky-300 font-bold block uppercase">
              {tipoMovimiento === 'apertura' ? 'CONTEO DE EMPAQUES (DESDE BD):' : `CANTIDADES DE EMPAQUES A REGISTRAR (${tipoMovimiento.toUpperCase()}):`}
            </span>
                ...
                {productosFiltradosVenta.length === 0 ? (
                  ...
                  productosFiltradosVenta.map((prod) => {
                    const esCorralito =
                      ...
                      (prod.nombre || '').toLowerCase().includes('wafleoso');
                ...
                {!esRappiActivo && (
                  <div className="bg-[#051829] border border-[#0066b3] p-2 rounded-xl space-y-1 text-xs">
                    <div className="flex justify-between text-sky-200">
                      <span>Ubicación:</span>
                      <b className="text-white">{itemActivoActual.nombre}</b>
                    </div>
                    <div className="flex justify-between text-sky-200">
                      <span>Subtotal:</span>
      ...
      {moduloActivo === 'cierre' && (
        <div className={`bg-[#0b2b48] border ${esTurnoManana ? 'border-amber-500/60' : 'border-purple-500/60'} p-5 rounded-2xl space-y-4 shadow-md max-w-3xl mx-auto ${bloqueadoPorApertura ? 'opacity-50 pointer-events-none' : ''}`}>
          <div className={`flex justify-between items-center border-b ${esTurnoManana ? 'border-amber-500/40' : 'border-purple-500/40'} pb-2`}>
            <h2 className={`text-sm font-black ${esTurnoManana ? 'text-amber-300' : 'text-purple-300'} flex items-center gap-1.5`}>
              {esTurnoManana ? '5. 🔄 Módulo de Cambio de Turno (Mañana)' : '5. 🌙 Módulo de Cierre de Turno y Cuadre de Caja'}
            </h2>
          </div>
          ...
          <div className="bg-[#051829] border border-purple-500/40 p-4 rounded-xl space-y-3">
            <span className="text-xs font-black text-purple-200 uppercase block border-b border-purple-500/30 pb-1">
              💸 Liquidación de Nómina del Turno ({sesion?.nombre || sesion?.nombre_completo || 'Iris'})
            </span>
          ...
          {esTurnoManana ? (
            <div className="bg-[#051829] border border-amber-500/50 p-4 rounded-xl space-y-3">
              <span className="text-xs font-black text-amber-300 uppercase block border-b border-amber-500/30 pb-1">
                🔄 Cambio de Turno (Mañana ➡️ Tarde)
              </span>
              <p className="text-xs text-sky-200">
                Una vez liquidada y pagada tu nómina, puedes proceder a contar el efectivo que dejas en caja y entregar el turno al operador entrante.
              </p>
              <button
                onClick={() => {
                  if (!nominaPagadaEnTurno) {
              ...
              <button
                onClick={() => {
                  if (!nominaPagadaEnTurno) {
                    const totalNominaCalculado = calcularTotalNomina();
                    if (totalNominaCalculado <= 0) {
                      alert('⚠️ NO PUEDES REALIZAR EL CIERRE:\n\nDebes liquidar y pagar obligatoriamente la nómina antes del cierre.');
                      return;
                    }
                  }
