      ...
      for (const item of itemsVendidos) {
        ...
        for (const emp of empaquesAfectados) {
          const empEncontrado = empaquesBD.find(
            (e) => e.nombre.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim() === emp.nombre.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim()
          );
          ...
          <div className="bg-[#0b2b48] border border-purple-500/60 p-5 rounded-2xl w-full max-w-4xl space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              ...
              <div className="bg-[#051829] border border-[#0066b3] p-4 rounded-xl space-y-2">
                ...
                <div className="flex justify-between pt-1 border-t border-[#0066b3]/30">
                  <span>Total Transacciones Electrónicas:</span>
                  <b className="text-sky-300">$ {totalVentasElectronicas.toLocaleString('es-CO')}</b>
                </div>
                <div className="flex justify-between">
                  <span>Total Descuentos Aplicados:</span>
                  <b className="text-amber-300">$ {totalDescuentosDia.toLocaleString('es-CO')}</b>
                </div>
                <div className="flex justify-between pt-2 border-t border-[#0066b3]/50 font-black text-sm">
                  <span className="text-white">Ventas Totales del Día:</span>
