...
const PUNTO_ID = 'centro'; 
const TABLA_PRODUCTOS = 'productos_centro';
const TABLA_SUMINISTROS = 'pedidos_centro';
...
export default function POSCentroPage() {
  ...
  async function cargarStockEspecifico() {
    ...
    try {
      ...
      if (error) {
        console.error('Error cargando productos de Centro:', error.message);
        setProductos([]);
      } else {
        setProductos(data || []);
      }
    } catch (e) {
      console.warn('Excepción cargando stock Centro:', e);
      setProductos([]);
    } finally {
 ...
 async function enviarSolicitudSuministro() {
  ...
  // No enviamos 'punto_id' porque la tabla es 'pedidos_centro'
  ...
  try {
    const { error } = await supabase.from(TABLA_SUMINISTROS).insert([nuevaSolicitud]);
    if (error) {
      console.error('Error enviando suministro a Centro:', error.message);
    }
  } catch (e) {
    console.error('Error enviando suministro a Centro:', e);
  }
  ...
  setAlerta({
    ...
    mensaje: `El pedido de suministros para ${PUNTO_ID.toUpperCase()} fue registrado en ${TABLA_SUMINISTROS}.`,
    tipo: 'exito',
  });
}
    ...
    <main className="min-h-screen w-screen bg-[#07090e] text-gray-100 p-3 sm:p-5 font-sans flex flex-col justify-between relative">
      <div className="flex-1 overflow-y-auto pb-24 pr-1 space-y-4">
        ...
        <header className="max-w-7xl mx-auto bg-[#0d111a] p-3.5 sm:p-4 rounded-2xl border border-gray-800 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-cyan-600 flex items-center justify-center text-xl shadow-md">
              🏢
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-black px-2 py-0.5 rounded-md bg-cyan-900/60 text-cyan-300 border border-cyan-700/50">
                  STOCK INDEPENDIENTE
                </span>
              </div>
        ...
        {!mesaSeleccionada ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {mesas.map((m) => (
                <div
                  ...
                  className="bg-[#0d111a] border border-gray-800 p-4 rounded-2xl cursor-pointer hover:border-cyan-500 transition-all text-center"
                >
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 bg-[#0d111a] p-4 rounded-2xl border border-gray-800 space-y-3">
              ...
              {/* PRODUCTOS */}
              {cargandoProductos ? (
                <p className="text-xs text-gray-500 text-center py-8">Cargando inventario de Centro...</p>
              ) : productosCatalogo.length === 0 ? (
                <p className="text-xs text-gray-500 text-center py-8">No se encontraron productos en productos_centro.</p>
              ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 bg-gray-900 p-1 rounded-xl border border-gray-800 shrink-0">
                          ...
                          <span className="font-black text-xs w-5 text-center text-cyan-400">{cantAgregada}</span>
                          <button
                            onClick={() => modificarCantidad(prod, 1)}
                            className="w-7 h-7 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-black text-sm flex items-center justify-center"
                          >
                            +
                          </button>
                        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 w-full max-w-7xl mx-auto bg-[#0a0d14]/95 backdrop-blur-md border-t border-gray-800/80 px-4 py-2.5 z-40">
        <div className="flex justify-around items-center text-center">
          <button onClick={() => setMesaSeleccionada(null)} className="flex flex-col items-center gap-0.5 text-cyan-400 font-bold">
      ...
      {alerta && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-[#0d111a] border border-gray-800 p-5 rounded-3xl max-w-xs w-full text-center space-y-3">
            ...
            <button onClick={() => { if(alerta.irAlLobbyAlCerrar) setMesaSeleccionada(null); setAlerta(null); }} className="w-full bg-cyan-600 text-white font-black py-2 rounded-xl text-xs">
              Entendido
            </button>
          </div>
        </div>
